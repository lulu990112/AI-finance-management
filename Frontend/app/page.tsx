...
export default function Home() {
  ...
  const handleSyncGmail = async () => {
    try {
      console.log('starting process Gmail sync...');
      console.log('Token before sync:', localStorage.getItem('authToken') ? 'exists' : 'not exists');
      ...
      console.log('✅ Gmail sync success, preparing to refresh data...');
      console.log('Token before refresh:', localStorage.getItem('authToken') ? 'exists' : 'not exists');
      ...
      console.log('Token status when error:', localStorage.getItem('authToken') ? 'exists' : 'not exists');
      
      if (error instanceof Error && error.message && error.message.includes('Authentication failed')) {
        setSyncError('Authentication failed, please login again');
        // only redirect to login page if not on login page
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 32 }}>
            ...
            <div style={{ flex: 1, minWidth: 260, display: "flex", flexDirection: "column", gap: 16, alignItems: "stretch", marginTop: 12 }}>
              ...
              >
                ...
                {syncLoading ? "Syncing..." : "Sync Gmail receipts"}
                <style jsx>{`
