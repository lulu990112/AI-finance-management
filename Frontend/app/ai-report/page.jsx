...
export default function AIReportPage() {
  ...
  const detailMap = {
    Needs: [
      ...
      "Household", "User defined"
    ],
    Wants: [
      // Red subcategory (wants) 
      "Clothing", "Shoes", "Electronics", "Cosmetic", "Snack", "Restaurant",
      "Game", "Movie", "KTV"
    ]
  };
  ...
  const defaultDonutData = [
    { name: "Needs", value: needsPercentage, color: "#9e9e9e" },
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={aiReport?.donut_data || defaultDonutData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={55} outerRadius={80}
                    labelLine={false}
                  >
                    {(aiReport?.donut_data || defaultDonutData).map((entry, idx) => <Cell key={entry.name} fill={entry.name === 'Needs' ? '#9e9e9e' : entry.color} />)}
                  </Pie>
                  ...
                  <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                    {needList.map(sub => (
                      <li key={sub.name} style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
                        <span style={{ display: "inline-block", minWidth: 80, fontWeight: 600, color: "#9e9e9e", background: "#9e9e9e22", borderRadius: 8, padding: "4px 14px", marginRight: 14 }}>{sub.name}</span>
